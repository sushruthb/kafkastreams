...
class KafkaServer() {
  ...
  private val LOGGER = Logger[KafkaServer]
  ...
  private var zookeeperHandle: Option[Zookeeper] = None
  private var kafkaBrokerHandle: Option[KafkaServerStartable] = None

  /**
   * First the Zookeeper and then the Kafka broker.
   */

  def start() {
    LOGGER.info(s"starting on [$zkPort $kbPort]")
    zookeeperHandle = Some(new Zookeeper(zkPort, tempDirs))
